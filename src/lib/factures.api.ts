...
const CREATE_FACTURES_FROM_BL = `
  mutation CreateFacturesFromBonLivraisons($bon_livraison_ids: [ID!]!) {
    createFacturesFromBonLivraisons(bon_livraison_ids: $bon_livraison_ids) {
      numero_facture
      factures {
        id
        numero_facture
...
export async function createFacturesFromBonLivraisons(
  bon_livraison_ids: Array<string | number>
) {
  return graphqlRequest<{
    createFacturesFromBonLivraisons: {
  ...
  }>(CREATE_FACTURES_FROM_BL, { bon_livraison_ids });
}
