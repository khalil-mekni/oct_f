...
const OrderTimelineDetail = ({
  ...
  const estEnRetard =
    joursRestants < 0 && item.statut !== "RECEPTIONNEE";
    ...
    <div className="bg-gray-50/80 p-8 border-t border-gray-100 animate-in slide-in-from-top duration-300">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="space-y-4">
          ...
          <div
            className={`p-5 rounded-[2rem] border-2 shadow-sm flex items-center gap-4 ${
              estEnRetard ? "bg-red-50 border-red-100" : "bg-white border-white"
            }`}
          >
            <div
              className={`h-12 w-12 rounded-2xl flex items-center justify-center ${
                estEnRetard ? "bg-red-500 text-white" : "bg-indigo-600 text-white"
              }`}
            >
            ...
            <div>
              ...
              <p
                className={`text-lg font-black ${
                  estEnRetard ? "text-red-600" : "text-gray-900"
                }`}
              >
                {joursRestants > 0
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-3">
            <div className="flex justify-between text-xs font-black">
              ...
              <span className="text-gray-900">
                {qteTotale} {emballageLabel}
              </span>
            </div>
            <div className="h-3 w-full bg-gray-100 rounded-full overflow-hidden">
              <div
                className={`h-full transition-all duration-1000 ${
                  resteARecevoir < 0 ? "bg-amber-500" : "bg-indigo-600"
                }`}
                style={{ width: `${ratio}%` }}
              />
            </div>
            <div className="flex justify-between items-center">
              <p className="text-[10px] font-bold text-gray-400 uppercase">
                Déjà reçu: <span className="text-indigo-600">{qteRecue}</span>
              </p>
              {resteARecevoir > 0 ? (
                <p className="text-[10px] font-bold text-amber-600 uppercase italic">
                  Reste: {resteARecevoir}
                </p>
              ) : resteARecevoir < 0 ? (
                <p className="text-[10px] font-bold text-red-600 uppercase italic">
                  Surplus: {Math.abs(resteARecevoir)}
                </p>
              ) : (
...
export default function CommandesTable({
  ...
  const contractStats = useMemo(() => {
    ...
    const pourcentage =
      total > 0 ? Math.min(((realise + saisie) / total) * 100, 100) : 0;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const el = document.getElementById(`commande-row-${highlightedId}`);
      if (el) {
  ...
  }, [highlightedId, expandedId, filteredRows]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      router.replace(queryString ? `/commandes?${queryString}` : "/commandes");
    }, 2500);
    ...
    <div className="min-h-screen bg-gray-50/50 p-4 lg:p-8 font-sans">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={openNew}
            className="flex items-center gap-2 rounded-2xl bg-indigo-600 px-6 py-3 text-sm font-black text-white shadow-xl shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all"
          >
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-black text-amber-600">
              {rows.reduce(
                (acc, curr) => acc + Math.max(0, Number(curr.reste || 0)),
                0
              )}
            </p>
          </div>
            ...
            <p className="text-xl font-black text-red-600">
              {
                rows.filter(
                  (r) =>
                    new Date(r.date_livraison_prevue) < new Date() &&
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-black text-green-600">
              {Math.round(
                (rows.filter((r) => r.statut === "RECEPTIONNEE").length /
                  rows.length) *
                  100 || 0
              )}
              %
                    ...
                    >
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2">
                          {isHighlighted && (
                            <span className="inline-flex rounded-full bg-red-100 px-2 py-0.5 text-[9px] font-bold uppercase text-red-700">
                              Alerte
                            </span>
                          )}
                        ...
                        <div className="text-[10px] text-gray-400 font-bold tracking-tighter uppercase">
                          {formatDate(item.date_commande)}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-gray-400 font-bold uppercase flex items-center gap-1 italic">
                      ...
                      <td className="px-6 py-5 text-center">
                        ...
                        <div className="text-[10px] mt-1 text-gray-400 font-bold uppercase">
                      ...
                      <td className="px-6 py-5">
                        <span
                          className={`inline-flex items-center rounded-full border px-4 py-1 text-[9px] font-black uppercase tracking-widest shadow-sm ${STATUT_STYLES[item.statut]}`}
                        >
                          {item.statut}
                        </span>
                      </td>
                      ...
                      >
                        <div className="flex justify-end gap-1">
                          <button
                            ...
                            className="p-3 text-gray-400 hover:bg-white hover:text-indigo-600 rounded-2xl shadow-none hover:shadow-md transition-all"
                          >
                          ...
                          {item.statut === "VALIDEE" && (
                            <button
                              ...
                              className="p-3 text-gray-400 hover:bg-white hover:text-amber-600 rounded-2xl transition-all"
                            >
                          ...
                          {item.statut === "EN_ATTENTE" && (
                            <button
                              ...
                              className="p-3 text-gray-400 hover:bg-white hover:text-red-600 rounded-2xl transition-all"
                            >
                    ...
                    {expandedId === item.id && (
                      <tr>
                        <td colSpan={6} className="p-0 border-none bg-white">
                          <OrderTimelineDetail
                            item={item}
                            emballageLabel={emballagesMap.get(
                              String(item.emballage_id)
                            )}
                          />
                        </td>
                      </tr>
          ...
          <div className="fixed inset-y-0 right-0 z-[1001] w-full max-w-md transform border-l border-gray-100 bg-white shadow-[-30px_0_80px_rgba(0,0,0,0.1)] transition-transform duration-500 ease-out rounded-l-[3.5rem] overflow-hidden">
            <div className="flex h-full flex-col">
              <div className="border-b border-gray-50 p-10 pb-6 flex items-center justify-between">
                ...
                <button
                  onClick={closeDrawer}
                  className="rounded-2xl h-12 w-12 flex items-center justify-center bg-gray-50 text-gray-400 hover:bg-gray-100 transition-all"
                >
                  <X />
                </button>
              </div>
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        emballage_id: e.target.value,
                        fournisseur_id: "",
                      })
                    }
                    className="w-full rounded-2xl border-2 border-gray-50 bg-gray-50 p-4 text-sm font-black focus:bg-white focus:border-indigo-600 outline-none transition-all cursor-pointer shadow-sm appearance-none"
                    required
                  >
                ...
                >
                  ...
                  <select
                    ...
                    className="w-full rounded-2xl border-2 border-gray-50 bg-gray-50 p-4 text-sm font-black focus:bg-white focus:border-indigo-600 transition-all cursor-pointer shadow-sm"
                    required
                  >
                    <option value="">
                      {form.emballage_id
                        ? "Choisir le fournisseur..."
                        : "En attente de l'emballage"}
                    </option>
                ...
                {activeContract && contractStats && (
                  <div className="rounded-[2.5rem] border-2 border-indigo-50 bg-indigo-50/30 p-8 space-y-4 shadow-inner">
                    <div className="flex justify-between items-end">
                      ...
                      >
                        {contractStats.pourcentage.toFixed(0)}%
                      </span>
                    </div>
                    <div className="h-3 w-full overflow-hidden rounded-full bg-white border border-indigo-100/50">
                      <div
                        className={`h-full transition-all duration-1000 ${
                          contractStats.depasse ? "bg-red-500" : "bg-indigo-600"
                        }`}
                        style={{ width: `${contractStats.pourcentage}%` }}
                      />
                    </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border-2 border-gray-50 p-4 text-sm font-black outline-none focus:border-indigo-600 transition-all shadow-sm"
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border-2 border-gray-50 p-4 text-sm font-black outline-none focus:border-indigo-600 transition-all shadow-sm font-mono"
                      required
                    />
                  </div>
                </div>
              ...
              <div className="border-t border-gray-50 p-10 flex gap-4 bg-white/80 backdrop-blur-md">
                <button
                  type="button"
                  onClick={closeDrawer}
                  className="flex-1 rounded-2xl py-4 text-[11px] font-black text-gray-400 hover:text-gray-900 transition-all uppercase tracking-[0.2em]"
                >
                  Annuler
                </button>
                ...
                <button
                  ...
                  disabled={
                    submitLoading || !!contractStats?.depasse || !form.fournisseur_id
                  }
