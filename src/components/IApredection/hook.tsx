...
function getDerniereConsommationConnue(
  ...
  for (let i = 0; i < 36; i++) {
    ...
    if (consommation > 0) {
      return { consommation, anneeBase: y, moisBase: m };
    }
  }

  return { consommation: 0, anneeBase: annee, moisBase: mois };
}
...
function buildHistoriqueMensuel(
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const total_sorties = stockHistory
      .filter((item) => {
        const d = new Date(item.date_stock);
        return item.sens === "S" && d.getFullYear() === y && d.getMonth() + 1 === m;
      })
      .reduce((sum, item) => sum + Number(item.quantite || 0), 0);
    ...
    const total_entrees = stockHistory
      .filter((item) => {
        const d = new Date(item.date_stock);
        return item.sens === "E" && d.getFullYear() === y && d.getMonth() + 1 === m;
      })
      .reduce((sum, item) => sum + Number(item.quantite || 0), 0);
...
export function useAIPrediction() {
  ...
  const runPrediction = useCallback(async (filters: DashboardFilters) => {
    ...
    try {
      ...
      const hist = buildHistoriqueMensuel(stockHistory, filters.annee, filters.mois);
      setHistorique(hist);

      if (consommationBase.consommation <= 0) {
        setError("Aucune consommation historique trouvée pour cet emballage et cet entrepôt.");
        setResult(null);
        return;
      }
...
consommation_mois: consommationBase.consommation,      });
      ...
      const quantitePredite = Number(
  predResp.prediction?.quantite_predite ?? predResp.quantite_predite ?? 0
);
  ...
  return { result, historique, isLoading, error, runPrediction, reset };
}