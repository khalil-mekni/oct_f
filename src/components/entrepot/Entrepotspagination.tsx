...
export function EntrepotsPagination({
  page,
  perPage,
  total,
  onPageChange,
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-4 border-t border-gray-50 bg-gray-50/30 px-6 py-4 sm:flex-row dark:border-gray-800 dark:bg-gray-800/20">
      ...
      <div className="flex items-center gap-1">
        ...
        <NavBtn onClick={() => onPageChange(page - 1)} disabled={page === 1} title="Précédent">
          <ChevronLeft size={13} />
        </NavBtn>
        ...
        <div className="flex items-center gap-1 px-1">
          {pages.map((p, i) =>
            p === "..." ? (
              <span key={`e-${i}`} className="flex h-8 w-6 items-center justify-center text-xs font-bold text-gray-300">
                ···
              </span>
            ) : (
              <button
        ...
        <NavBtn onClick={() => onPageChange(page + 1)} disabled={page === totalPages} title="Suivant">
          <ChevronRight size={13} />
        </NavBtn>
        <NavBtn onClick={() => onPageChange(totalPages)} disabled={page === totalPages} title="Dernière page">
