...
export default function MouvementsPage() {
  ...
  async function handleDelete(id: string) {
    const ok = window.confirm("Voulez-vous vraiment supprimer ce mouvement ?");
    if (!ok) return;
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FC] p-4 lg:p-8">
      <div className="mx-auto max-w-[1600px] space-y-8">
                  ...
                  filteredItems.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-6">
                        ...
                        >
                          <span
                            className={`mr-1.5 h-1.5 w-1.5 rounded-full ${item.statut === "VALIDE"
                                ? "bg-[#00A09D]"
                                : "bg-amber-500"
                              }`}
                          />
                        ...
                        {item.statut === "BROUILLON" ? (
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              ...
                              className="rounded-full border border-red-200 bg-red-50 px-5 py-2 text-[10px] font-black uppercase tracking-widest text-red-700 transition hover:bg-red-100"
                            >
                              Supprimer
                            </button>
                          </div>
        ...
        {!loading && lastPage > 1 && (
          <div className="flex items-center justify-center gap-2 rounded-[2rem] border border-gray-100 bg-white py-4 shadow-sm">
            <button
              onClick={() => setPage((p) => Math.max(1, p - 1))}
              disabled={page === 1}
              className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 transition-all hover:bg-gray-50 disabled:opacity-30"
            >
              <ChevronLeft size={18} />
            </button>
            ...
            {Array.from({ length: lastPage }, (_, i) => i + 1).map((pageNum) => (
              <button
                ...
                className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-medium transition-all ${page === pageNum
